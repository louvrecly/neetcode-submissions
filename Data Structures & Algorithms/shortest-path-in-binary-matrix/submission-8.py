class Solution:
    def shortestPathBinaryMatrix(self, grid: List[List[int]]) -> int:
        ...
        n = len(grid)
        options = deque([(0, 0, 0)])  # (r, c, length)
        visited = set()
        minLength = n * n + 1
        ...
        while options:
            r, c, length = options.popleft()
            if r < 0 or r > n - 1 or c < 0 or c > n - 1 or grid[r][c] == 1 or (r, c) in visited:
                continue
            ...
            for deltaR, deltaC in deltas:
                options.append((r + deltaR, c + deltaC, length + 1))

        return minLength if minLength <= n * n else -1
        # # BFS with queue
        # n = len(grid)
        ...
        # def bfs(r: int, c: int, visited: Set[Tuple[int, int]]) -> int:
        #     visited.add((r, c))
        ...
        #         if (
        #             0 <= nextR < n and 0 <= nextC < n and
        #             grid[nextR][nextC] == 0 and
        #             not (nextR, nextC) in visited
        #         ):
        #             options.append((nextR, nextC))

        #     pathLength = n * n + 1
        #     while options:
        #         nextR, nextC = options.popleft()
        #         currLength = bfs(nextR, nextC, visited)
        #         pathLength = min(pathLength, currLength)

        #     return pathLength

        # length = bfs(0, 0, set())
        # return length if length <= n * n else -1
        # # BFS with queue
        # if grid[0][0] != 0 or grid[-1][-1] != 0:
        #     return -1
